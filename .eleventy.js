...
module.exports = function(config) {
    ...
    function createLeaderboards(collection, questFilter, weaponFilter, ruleset) {
        ...
        rulesetFilter = [ 'freestyle', 'ta-wiki-rules' ];
        switch (ruleset) {
            case 'ta-wiki-rules':
                rulesetFilter = [ 'ta-wiki-rules' ];
                break;
            case 'freestyle':
                rulesetFilter = [ 'freestyle', 'ta-wiki-rules' ];
                break;
            ...
            switch (item.data.type) {
                ...
                case 'run':
                    ...
                    if (item.data.runners.length == 1
                        && item.data.weapons.length == 1
                        && (weaponFilter.length == 0 || weaponFilter == item.data.weapons[0])
                        && rulesetFilter.includes(item.data.run_type)
                        ) {
                        // NOTE: FILESLUG IS NOT UNIQUE
                        if (questFilter.includes(item.data.quest)) {
                            runs.push(item);
                        }
                    }
                    break;
                case 'quest':
                    ...
                    //*/
                    if (questFilter.includes(item.fileSlug)) {
                        quests[item.fileSlug] = item;
                    }
                    break;
                case 'weapon':
